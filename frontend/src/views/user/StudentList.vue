<template>
  <div class="page-container">
    ...
    <div class="search-bar">
      <el-input v-model="query.keyword" placeholder="搜索姓名/用户名/手机号/学号" clearable style="width: 280px" @keyup.enter="loadData" />
    ...
    <el-table :data="tableData" v-loading="loading" stripe style="width: 100%" :header-cell-style="headerStyle">
      <el-table-column label="头像" width="70" align="center">
        <template #default="{ row }">
          <el-avatar :size="36" :src="row.avatar">
            <span>{{ row.realName?.charAt(0) || row.username?.charAt(0) }}</span>
          </el-avatar>
        </template>
      </el-table-column>
      <el-table-column prop="username" label="用户名" min-width="120" />
      <el-table-column prop="realName" label="姓名" width="100">
        <template #default="{ row }">{{ row.realName || '未填写' }}</template>
      </el-table-column>
      <el-table-column prop="studentId" label="学号" width="130">
        <template #default="{ row }">{{ row.studentId || '未填写' }}</template>
      </el-table-column>
      <el-table-column prop="phone" label="手机号" width="130" />
      <el-table-column prop="dormAddress" label="宿舍地址" min-width="150">
        <template #default="{ row }">{{ row.dormAddress || '未填写' }}</template>
      </el-table-column>
      <el-table-column prop="reputation" label="信誉分" width="90" align="center">
        <template #default="{ row }">
          <el-tag :type="row.reputation >= 80 ? 'success' : row.reputation >= 60 ? 'warning' : 'danger'">
            {{ row.reputation }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="认证" width="80" align="center">
        <template #default="{ row }">
          <el-tag :type="row.isCertified ? 'success' : 'info'" size="small">
...
async function loadData() {
  loading.value = true
  try {
    const res = await userApi.getStudents(query)
...
function resetQuery() {
  query.keyword = ''; query.status = null; query.current = 1
  loadData()
}

async function handleBan(row) {
  await ElMessageBox.confirm(`确定封禁用户【${row.realName || row.username}】吗？`, '警告', { type: 'warning' })
  await userApi.banUser(row.id)
